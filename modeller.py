...
class ModelConfig:
    ...
    target_col: str = 'fp_Rice Yield (kg/ha)'
    group_cols: List[str] = None
    test_size: float = 0.2
    cv_folds: int = 5
    random_state: int = 887991
...
class UniCropModeler:
    ...
    def __init__(self, config: ModelConfig = None):
        ...
        # Figures folder
        os.makedirs("unicrop_figures1", exist_ok=True)
    ...
    def load_and_validate_data(self, filepath: str) -> pd.DataFrame:
        ...
        categorical_cols = ['fp_District', 'fp_Season(SA = Summer Autumn, WS = Winter Spring)',
    ...
    def comprehensive_eda(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        plt.tight_layout()
        plt.savefig("unicrop_figures1/01a_target_distribution.svg", bbox_inches="tight")
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig("unicrop_figures1/01b_qq_plot.svg", bbox_inches="tight")
        plt.close()

        # === Yield by Season (separate figure) ===
        season_col = "fp_Season(SA = Summer Autumn, WS = Winter Spring)"
        if season_col in df.columns:
            ...
            plt.tight_layout()
            plt.savefig("unicrop_figures1/01c_yield_by_season.svg", bbox_inches="tight")
            plt.close()

        # === Yield by District (separate figure) ===
        if "fp_District" in df.columns:
            plt.figure(figsize=(10, 6))
            sns.boxplot(data=df, x="fp_District", y=target, width=0.6, fliersize=2.5, linewidth=1.6)
            plt.title("Yield by District")
            plt.xticks(rotation=30)
            plt.grid(True, axis="y", alpha=0.25)
            plt.tight_layout()
            plt.savefig("unicrop_figures1/01d_yield_by_district.svg", bbox_inches="tight")
            plt.close()
        ...
        if len(top_missing) > 0:
            ...
            plt.tight_layout()
            plt.savefig("unicrop_figures1/02a_top_missingness.svg", bbox_inches="tight")
            plt.close()
        ...
        if vi_cols and season_col in df.columns:
            ...
            plt.tight_layout()
            plt.savefig("unicrop_figures1/02b_vi_missing_by_season.svg", bbox_inches="tight")
            plt.close()
        ...
        for fam, feats in feature_families.items():
            ...
            if len(fam_num) > 1:
                ...
                plt.tight_layout()
                fname = f"unicrop_figures1/03_{fam.lower()}_correlations.svg".replace(' ', '_')
    ...
    # ----------------------------------
    def _apply_preprocessing(self, df: pd.DataFrame, is_train: bool = True, preprocessors: Dict = None) -> Tuple[pd.DataFrame, Dict]:
        df_processed = df.copy()
        local_preprocessors = {} if preprocessors is None else dict(preprocessors)

        # 3.1 Winsorization (per-fold)
        numeric_cols = [c for c in df_processed.select_dtypes(include=[np.number]).columns if c != self.config.target_col]
        if is_train:
            winsor_limits = {}
            for col in numeric_cols:
        ...
        meteo_cols = [c for c in df_processed.columns if any(m in c.upper() for m in ['T2M', 'RH2M', 'VPD', 'WS2M', 'ALLSKY', 'DTR', 'DEW', 'ET'])]
        meteo_cols = [c for c in meteo_cols if c in numeric_cols]
        if meteo_cols:
            if is_train:
                meteo_imputer = IterativeImputer(estimator=BayesianRidge(), random_state=self.config.random_state, max_iter=10)
        ...
        vi_cols = [c for c in df_processed.columns if any(v in c.upper() for v in ['NDVI', 'EVI', 'LAI', 'NDRE', 'SAVI', 'CIREDGE', 'GPP', 'FPAR'])]
        ...
        if vi_cols:
            ...
            for cat_col in ['fp_District', 'fp_Season(SA = Summer Autumn, WS = Winter Spring)']:
                if cat_col in df_processed.columns:
                    ...
                    if is_train:
                        ...
                        le = local_preprocessors[f'le_{cat_col}']
            ...
            if context_features:
                ...
                if is_train:
                    ...
                    vi_imputed = local_preprocessors['vi_imputer'].transform(vi_features)
                df_processed[vi_cols] = vi_imputed[:, :len(vi_cols)]
        local_preprocessors['vi_cols'] = vi_cols

        soil_cols = [c for c in df_processed.columns if any(s in c.lower() for s in
                       ['ocd_', 'nitrogen_', 'phh2o_', 'sand_', 'silt_', 'clay_', 'bdod_', 'elevation', 'slope', 'aspect'])]
        ...
        cat_cols = ['fp_District', 'fp_Season(SA = Summer Autumn, WS = Winter Spring)',
                    'fp_Rice Crop Intensity(D=Double, T=Triple)']
        encoded_dfs = []
        for col in cat_cols:
            if col in df_processed.columns:
        ...
        if is_train:
            local_preprocessors['one_hot_columns'] = list(enc.columns) if encoded_dfs else []

        # 3.4 Scaling
        final_numeric = [c for c in df_processed.select_dtypes(include=[np.number]).columns if c != self.config.target_col]
        if is_train:
            ...
            scaler = local_preprocessors['scaler']
    ...
    def feature_engineering(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if numeric_cols:
            ...
            if len(top) >= 4:
                ...
                interactions = poly.fit_transform(df_eng[top[:4]])
                names = poly.get_feature_names_out(top[:4])
    ...
    def train_baseline_models(self, df: pd.DataFrame) -> Dict:
        ...
        for fold, (tr_idx, va_idx) in enumerate(cv.split(df, df[target], groups)):
            ...
            for C in [1, 5, 10]:
                for gamma in ['scale', 0.05, 0.1]:
                    svr = SVR(kernel='rbf', C=C, epsilon=0.1, gamma=gamma)
                    svr.fit(Xtr, ytr)
                    p = svr.predict(Xva)
        ...
        for name in oof:
            ...
            metrics[name] = {
                ...
                'MAPE': float(np.mean(np.abs((y_full - yp) / np.clip(y_full, 1e-6, None))) * 100)
            }
        ...
        for metric in ['RMSE', 'MAE', 'R2', 'MAPE']:
            if metric in mdf.columns:
                ...
                plt.tight_layout()
                plt.savefig(f'unicrop_figures1/05{dict(RMSE="a",MAE="b",R2="c",MAPE="d")[metric]}_{metric.lower()}_comparison.svg',
                            bbox_inches='tight')
                plt.close()
        ...
        plt.tight_layout()
        plt.savefig('unicrop_figures1/05e_oof_scatter.svg', bbox_inches='tight')
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig("unicrop_figures1/05f_residual_diagnostics.svg", bbox_inches='tight')
        plt.close()

        # # ==================================================
        # # FIGURE I – ROBUSTNESS BY SEASON AND DISTRICT (WITHIN-SAMPLE)
        # # ==================================================
        # # We compute RMSE per season and per district, using the FINAL OOF PREDICTIONS.
        # # This ensures values are in ORIGINAL YIELD UNITS (no scaling mismatch).
        #
        ...
        #
        # # -----------------------------------------
        # # 1) Identify the best model and its OOF preds
        # # -----------------------------------------
        #
        # # If your metrics dict already includes the best model name:
        # best_model = None
        # if "best_model" in metrics:
        #     best_model = metrics["best_model"]
        # elif "best_model_name" in metrics:
        #     best_model = metrics["best_model_name"]
        # else:
        #     # fallback: pick the best R² score
        #     best_model = max(metrics.keys(), key=lambda k: metrics[k]["R2"])
        #
        # print(f"➡️ Using OOF predictions from BEST MODEL: {best_model}")
        #
        # # Retrieve BEST MODEL OOF predictions
        # y_pred = oof[best_model]
        # y_true = y_full  # same actual values used in Fig H (original scale)
        #
        # # -----------------------------------------
        # # 2) Prepare dataframe for grouping
        # # -----------------------------------------
        # df_plot = df.copy()  # dataframe with season/location columns
        ...
        #
        # # -----------------------------------------
        # # 3) Compute robustness by SEASON
        # # -----------------------------------------
        # season_metrics = None
        # if season_col and (season_col in df_plot.columns):
        #     grouped = df_plot.groupby(season_col)
        #     season_metrics = pd.DataFrame({
        #         "group": grouped[season_col].first(),
        #         "RMSE": grouped.apply(lambda g: rmse(g["y_true"], g["y_pred"])),
        #         "MAE": grouped["abs_error"].mean(),
        #         "R2": grouped.apply(lambda g: r2_score(g["y_true"], g["y_pred"])),
        #         "type": "Season"
        #     })
        # else:
        #     print("⚠️ No season column available for seasonal robustness.")
        #
        # # -----------------------------------------
        # # 4) Compute robustness by DISTRICT
        # # -----------------------------------------
        # district_metrics = None
        # if district_col and (district_col in df_plot.columns):
        #     grouped = df_plot.groupby(district_col)
        #     district_metrics = pd.DataFrame({
        #         "group": grouped[district_col].first(),
        #         "RMSE": grouped.apply(lambda g: rmse(g["y_true"], g["y_pred"])),
        #         "MAE": grouped["abs_error"].mean(),
        #         "R2": grouped.apply(lambda g: r2_score(g["y_true"], g["y_pred"])),
        #         "type": "District"
        #     })
        # else:
        #     print("⚠️ No district column available for district robustness.")
        #
        # # -----------------------------------------
        # # 5) PLOT FIGURE I
        # # -----------------------------------------
        # fig_I = make_subplots(
        #     rows=1, cols=2,
        #     subplot_titles=("RMSE by Season", "RMSE by District"),
        ...
        #     fig_I.add_trace(
        #         go.Bar(
        #             x=season_metrics["group"],
        #             y=season_metrics["RMSE"],
        #             marker=dict(color="#1f77b4"),
        #             name="Season RMSE"
        #         ),
        #         row=1, col=1
        #     )
        #     fig_I.update_xaxes(title_text="Season", row=1, col=1)
        #     fig_I.update_yaxes(title_text="RMSE (kg/ha)", row=1, col=1)
        # else:
        #     print("⚠️ Season metrics missing: cannot plot left panel of Figure I.")
        #
        # # ----- RIGHT: District RMSE -----
        # if district_metrics is not None and not district_metrics.empty:
        #     fig_I.add_trace(
        #         go.Bar(
        #             x=district_metrics["group"],
        #             y=district_metrics["RMSE"],
        #             marker=dict(color="#ff7f0e"),
        #             name="District RMSE"
        #         ),
        #         row=1, col=2
        #     )
        #     fig_I.update_xaxes(title_text="District", row=1, col=2)
        #     fig_I.update_yaxes(title_text="RMSE (kg/ha)", row=1, col=2)
        # else:
        #     print("⚠️ District metrics missing: cannot plot right panel of Figure I.")
        #
        # # Final styling
        # fig_I.update_layout(
        ...
        # )
        #
        # fig_I.show(renderer="browser")
        # fig_I.write_image("figure_I_robustness.png", scale=3)
        ...
        print("\n✅ Final 15 features selected for the model:")
    ...
    def plot_all_final_diagnostics(self, df, selection_summary, best_model_name, feature_importance_df):
        ...
        plt.tight_layout()
        plt.savefig('unicrop_figures1/04a_composite_relevance.svg', bbox_inches='tight')
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig('unicrop_figures1/04b_family_coverage.svg', bbox_inches='tight')
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig('unicrop_figures1/04c_final_selected.svg', bbox_inches='tight')
        plt.close()
        ...
        if best_model_name in ['LightGBM', 'RandomForest'] and not feature_importance_df.empty:
            ...
            plt.tight_layout()
            plt.savefig('unicrop_figures1/04d_feature_importance.svg', bbox_inches='tight')
            plt.close()
    ...
    def ensemble_models(self, df: pd.DataFrame) -> Dict:
        ...
        met = {
            ...
            'MAPE': float(np.mean(np.abs((y - oof_ens) / np.clip(y, 1e-6, None))) * 100)
        }
        ...
        for metric in ['RMSE', 'MAE', 'R2', 'MAPE']:
            if metric in mdf.columns:
                ...
                plt.grid(True, alpha=0.3)
                plt.tight_layout()
                plt.savefig(f'unicrop_figures1/06{dict(RMSE="a",MAE="b",R2="c",MAPE="d")[metric]}_{metric.lower()}_comparison.svg',
                            bbox_inches='tight')
                plt.close()
    ...
    def model_interpretability(self, df: pd.DataFrame, selected_features: List[str]) -> Dict:
        ...
        if 'LightGBM_full' in self.models:
            ...
            plt.tight_layout()
            plt.savefig('unicrop_figures1/07_shap_summary.svg', bbox_inches='tight')
            plt.close()
            ...
            plt.tight_layout()
            plt.savefig('unicrop_figures1/08_shap_importance.svg', bbox_inches='tight')
            plt.close()
            ...
            for i, feat in enumerate(top_feats, start=1):
                ...
                plt.tight_layout()
                plt.savefig(f'unicrop_figures1/09{i}_shap_dependence_{i}.svg', bbox_inches='tight')
                plt.close()
    ...
    def generate_prediction_function(self, df: pd.DataFrame, selected_features: List[str]) -> None:
        ...
        # Save PKL
        with open('unicrop_model_artifacts1.pkl', 'wb') as f:
...
def predict_crop_yield(df_raw):
    ...
    return preds, qa
'''
        with open('predict_crop_yield.py', 'w') as f:
...
"""
        ...
        if 'final_comparison' in self.artifacts:
            ...
            for model_name, metrics in self.artifacts['final_comparison'].items():
                report += f"| {model_name} | {metrics['RMSE']:.4f} | {metrics['MAE']:.4f} | {metrics['R2']:.4f} | {metrics['MAPE']:.2f}% |\n"
...
"""
        ...
        with open('unicrop_final_report.md', 'w') as f:
            f.write(report)
...
if __name__ == "__main__":
    # Actual dataset path
    data_filepath = "unicrop_master_timeseries.csv"
