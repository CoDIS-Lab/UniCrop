...
class UniCropModeler:
    ...
    def train_baseline_models(self, df: pd.DataFrame) -> Dict:
        ...
        jaccard_scores = [
            jaccard(f1, f2)
            for f1, f2 in combinations(selected_features_per_fold, 2)
        ]

        if jaccard_scores:
            self.artifacts['selected_features_per_fold'] = selected_features_per_fold
            self.artifacts['feature_selection_stability'] = {
                'mean_jaccard': float(np.mean(jaccard_scores)),
                'min_jaccard': float(np.min(jaccard_scores)),
                'max_jaccard': float(np.max(jaccard_scores)),
                'all_jaccard_scores': [float(x) for x in jaccard_scores]
            }
